export class DataBuffer {
  ...
  public static wrap(array: Uint8Array | ArrayBuffer, littleEndian?: boolean) {
    ...
    if (array instanceof ArrayBuffer) result.data = new DataView(array);
      ...
      throw new TypeError("Unexcepted type for array param");
    }
  ...
  public writeUint8(number: number, position?: number): DataBuffer {
    const offset = position === undefined ? this.offset : position;
    this.checkBounds(offset, this.limit, 1);
  ...
  public writeInt8(number: number, position?: number): DataBuffer {
    const offset = position === undefined ? this.offset : position;
    this.checkBounds(offset, this.limit, 1);
  ...
  public writeInt16(number: number, position?: number): DataBuffer {
    const offset = position === undefined ? this.offset : position;
    this.checkBounds(offset, this.limit, 2);
  ...
  public writeUint16(number: number, position?: number): DataBuffer {
    const offset = position === undefined ? this.offset : position;
    this.checkBounds(offset, this.limit, 2);
  ...
  public writeInt32(number: number, position?: number): DataBuffer {
    const offset = position === undefined ? this.offset : position;
    this.checkBounds(offset, this.limit, 4);
  ...
  public writeUint32(number: number, position?: number): DataBuffer {
    const offset = position === undefined ? this.offset : position;
    this.checkBounds(offset, this.limit, 4);
  ...
  public readCStringAsArray(position?: number): DataBuffer {
  ...
  public readCString(position?: number): string {
    return DataBuffer.stringFromUTF8Array(
      this.readCStringAsArray(position).toBuffer()
    );
  }

  public writeCString(str: string, position?: number): DataBuffer {
    const data = Uint8Array.of(...DataBuffer.toUTF8Array(str), 0);
  ...
  public readBytes(size: number, position?: number): DataBuffer {
    const offset = position === undefined ? this.offset : position;
  ...
  public append(data: Uint8Array | DataBuffer, position?: number) {
    const offset = position === undefined ? this.offset : position;

    if (data instanceof DataBuffer) {
      this.append(data.slice().toBuffer(), position);
    } else if (data instanceof Uint8Array) {
      this.checkBounds(offset, this.limit, data.byteLength);

      const thisArray = new Uint8Array(this.data.buffer);
      thisArray.set(data, offset);
      if (position === undefined) this.offset += data.byteLength;
    }
  }
